...
system_prompt = SystemMessagePromptTemplate.from_template(
    """
        ...
        <se não tiver informações suficientes para fornecer uma resposta curta, se tiver várias respostas possíveis ou se verificar que o pedido do usuário pode ou precisa ser armazenado em banco>
"""
)
...
prompt = ChatPromptTemplate.from_messages([
    ...
    HumanMessagePromptTemplate.from_template("{usuario}")
])
...
while True:
    try:
        usuario_input = input("Digite uma pergunta (ou 'sair' para encerrar): ")
        if usuario_input.lower() == "sair":
            print("Encerrando o assistente...")
            break

        resposta = chain.invoke(
            {"usuario": usuario_input},  # input principal
            config={"configurable": {"session_id": "Qualquer coisa"}}  # config separado
        )
